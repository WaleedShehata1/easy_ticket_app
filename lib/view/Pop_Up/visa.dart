...
import 'package:easy_ticket_app/view/screen/bottom_bar.dart';
    ...
    return MultiBlocProvider(
      ...
      child: BlocConsumer<AppCubit, AppState>(
        listener: (context, state) {
          ...
          } else if (widget.isWallet == false) {
            if (state is PayVisaSuccessState) {
              if (state.payVisa!.status != false) {
                print("valueScreen==${state.payVisa}");
                Navigator.pop(context);
                showToast(
                    text: state.payVisa!.message!, state: ToastStates.success);
              } else if (state.payVisa!.status == false) {
                showToast(
                    text: state.payVisa!.message!, state: ToastStates.error);
              } else {
                showToast(
                    text:
                        'There is a problem connecting to the server or the Internet',
                      ...
                      OnTap: () {
                        if (formKey.currentState!.validate()) {
                          if (widget.isWallet == true) {
                            ...
                            print('0000 ${ticket!['id']}');
                            AppCubit.get(context).payvisa(
                                The_owner_of_the_visa: cardHolderName,
